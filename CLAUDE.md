...
### 새 컨텍스트로 작업을 이어받을 때 (필수)

① `HANDOFF.md`의 "0. 최신 상태"로 현재 위치·다음 할 일을 먼저 파악한다. ② 이번에 손댈 영역이 완료 Step 표에서 ADR / learning / troubleshooting 포인터를 가지면, **그 문서를 먼저 읽고 시작**한다. HANDOFF "0. 최신 상태"는 진척 스냅샷일 뿐, 결정의 _왜_(제약·트레이드오프·폐기된 접근)는 ADR이 단일 출처다. 특히 ADR은 위반 시 회귀(이미 폐기한 접근의 부활 등)를 유발하므로 해당 영역 작업 전 필독한다. ADR을 통독할 필요는 없고, 건드리는 영역의 포인터만 on-demand로 따라가면 된다.
...
처리 방식: 갱신은 **머지 후**에 한다. "0. 최신 상태"는 머지된 사실(PR 번호·머지 완료)을 기록하므로 작업 PR에 동승시키면 선기록이 되어 후행 보정이 또 필요해진다(선례: PR #52·#53 in-PR 갱신에도 #54 후행 보정 발생). 같은 시점에 나가는 docs PR(학습 문서 등)이 있으면 거기에 합승시키고, 없을 때만 소형 전용 docs PR로 처리한다.
...
- **처리 방식**: 해당 PR에서 학습 내용이 완결되면 **작업 브랜치에 `docs:` 커밋으로 동승**시켜 함께 머지한다(후행 별도 PR로 빼지 않음 — 백필 누락 예방). 학습 주제가 여러 Step에 걸쳐야 완결되는 경우(예: `step9-web-push.md`는 발송이 붙는 9-c 시점)만 완결 Step에서 분리 작성한다
