...
```ts
$('ul.mainBoard_list > li').each((_, el) => {
  const $li = $(el); // raw element를 cheerio 객체로 감싸기
...
메인 페이지에는 **최신 5건만** 표시된다. 크롤링 주기(1시간) 사이에 6건 이상 등록되면 중간 공고가 메인에서 밀려 누락된다.

```
이전 크롤링 후: lastBoardId = 6477
그 사이 7건 등록: 6478 ~ 6484, 6485
현재 메인(5건): [6485, 6484, 6483, 6482, 6481] 만 등록되게 됨.

→ "메인에 있고 6477보다 큰 것"만 보면 6478, 6479, 6480은 영영 누락
```

### 해결 아이디어

`boardId`는 **자동 증가 정수**다. 따라서 `lastBoardId + 1` ~ `메인 최대 ID` 범위의 **모든 정수**를 후보로 삼으면, 메인에서 밀려난 ID도 놓치지 않는다.

```
lastBoardId = 6477,  메인 = [6485, 6479, 6478, 6477, 6474]

후보 = 6478 ~ 6485 (빈 번호 포함 전 범위)
  ├─ 확정 (메인에 있음):   6478, 6479, 6485  → 바로 DB 저장
  └─ 검증 필요 (메인에 없음): 6480, 6481, 6482, 6483, 6484
      → view.do?boardId=N 으로 실제 존재 확인
```

"검증 필요" ID는 세 가지 상태 중 하나: (1) 메인에서 밀려난 진짜 공고, (2) 삭제/비공개, (3) 결번. 상세 페이지 응답으로 판별한다.

### 두 단계 분리 (파서 ↔ 서비스)

```ts
// ── 파서 레이어 (순수 함수, 네트워크 없음) ──
...
// ── 서비스 레이어 (HTTP, 재시도, rate limit) ──
// needsVerification 은 여기서 fetch
```

순수 함수로 떼어내면 네트워크 모킹 없이 엣지 케이스(빈 배열, 연속 ID, 모두 알려진 경우)를 빠르게 테스트할 수 있다. HTTP 부수효과는 한 레이어에 모아 일관되게 관리.
