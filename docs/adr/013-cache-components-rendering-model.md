...
- **미검증**: remote 캐시 핸들러를 호스팅 제공자가 자동 구성한다는 것은 문서 서술에 의존한다("hosting providers should typically provide this automatically"). 실제 적중은 Vercel 프리뷰/프로덕션에서 확인해야 한다. 적중하지 않아도 기능은 정상이며 매 요청 DB 조회로 퇴화할 뿐이다.
