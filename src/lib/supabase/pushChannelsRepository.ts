 ...
 *
 * - getEnabledChannels: L1 push_preferences.enabled = true인 계정의
 *   L2 push_subscriptions 채널 전부를 조회한다 (ADR 008: "L1 enabled인
 *   계정의 모든 L2 채널로 발송").
 *   push_preferences ↔ push_subscriptions 사이에 직접 FK가 없어(둘 다
...
const PREFERENCES_TABLE = 'push_preferences';
...
/**
 * 구독 의사가 켜진(L1 enabled) 계정들의 배달 채널(L2)을 모두 조회한다.
 *
 * enabled 계정이 없거나 채널이 하나도 없으면 빈 배열.
